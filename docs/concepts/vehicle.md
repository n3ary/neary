...
| `verified` | Schedule + 2+ live sources agree. Currently unreachable in production because only one live source (GTFS-RT) is wired; kept in the type so multi-source (see [multi-source-live-data.md](../specs/multi-source-live-data.md)) can promote to it. |
