...
- **Cross-repo math sharing is deferred.** neary-gtfs already maintains
  ...
  *"vendored copy"*) and it has held up fine. No mirror tooling
  until the duplication actually bites. See item 7.
...
Build the cascade inline in `src/lib/domain/` for now; the cross-repo
sharing infrastructure (item 7) is a refactor for later, not a blocker.
...
### [ ] 6. Empirical per-segment speeds
...
#### [ ] 6c. Ship empirical data in SQLite
...
- [ ] Worker query exposes per-bucket lookup to the cascade consumer.
...
### [ ] 7. Shared cascade math between repos

**Deprioritized.** Same math lives in two places today:

- **Web app**: `src/lib/domain/shapeProjection.ts`, `distance.ts`, plus
  the inline cascade from item 2 once it lands.
- **neary-gtfs**: `src/pipeline/lib/polyline.js` (header explicitly
  reads *"vendored copy of neary/src/lib/domain/shapeProjection.ts"*),
  TOD-bucket logic inline in `feeds/cluj-napoca/lib/timing.js`.

Manual mirroring has held up — math doesn't change often, headers flag
the relationship. Formalize sharing only when the duplication actually
bites.

When we do:

- [ ] Establish `src/lib/domain/prediction-core/` as source of truth
      (TS): `geo.ts`, `timeOfDay.ts`, `speedCascade.ts`, `dwell.ts` +
      tests.
- [ ] Mirror to `neary-gtfs/src/pipeline/lib/prediction-core/` as plain
      ESM JS via `scripts/sync-prediction-core.mjs` (TS → JS strip).
- [ ] `scripts/check-mirror.{sh,mjs}` in CI fails any PR that touches
      one copy without the other.
- [ ] Replace the inline cascade in item 2 with imports from
      `prediction-core`.
- [ ] Replace the inline speed-bucket call in
      ...
      time). No behavioural change expected.
...
Items 6a–6c above live in neary-gtfs. Cross-referenced here so a
...
- No always-on historical service in the release path (item 6 is opt-in,
  runs manually).
- No multi-feed merging at runtime.
- No bypass of the worker for SQLite access.
- No formal cross-repo sharing infrastructure (sync scripts, CI mirror
  checks) until manual duplication actually hurts. See item 7.
