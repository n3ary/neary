 ...
 *
 * Navigating to /favorites or /settings does NOT reset - returning
 * from those tabs to / restores the rider's previous expansion +
 * route filter. Drilldowns (/map/..., /schedule/...) also don't
 * reset. Manual refresh (header refresh button) is NOT a reset
 * trigger either - the rider asked for fresh data, not for their
 * view to be wiped.
 */
...
class StationsViewStore {
   ...
   *  `expandedStopId` by `resetUserChoices` (and by `reset`). */
