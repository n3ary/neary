...
// Dev/debug console hooks. Lets the user pin a fake GPS location from
  ...
  // Tab-swap reset. When the rider navigates to /favorites or /settings
  // from the Stations view, wipe the view-state store so a return-to-/
  // renders the selector's auto-pick rather than the stale pick from
  // a context they explicitly left. Drilldowns (/map/... /schedule/...)
  // intentionally do NOT trigger this - back-and-forth within the
  // stations context should preserve expansion and route filters.
  // See issue #203.
  $effect(() => {
    const path = page.url.pathname;
    if (path === '/favorites' || path === '/settings') {
